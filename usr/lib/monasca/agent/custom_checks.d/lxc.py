    ...
    def _get_mem_metrics(self, container_name):
        ...
        with open('{0}/memory.usage_in_bytes'.format(mem_cgroup)) as mem_file:
            for line in mem_file:
                metrics['memory.usage_in_bytes'] = int(line)
        return metrics

    def _get_swap_metrics(self, container_name):
        """Get swap metrics from memory cgroup file. If swapaccount is defined,
           you can control swap memory. To active swap memory control, set
           GRUB_CMDLINE_LINUX_DEFAULT="swapaccount=1" on /etc/default/grub file
        ...
        if os.path.isfile(swap_file):
            with open(swap_file) as mem_file:
                for line in mem_file:
                    metrics['memory.memsw.usage_in_bytes'] = int(line)
        else:
